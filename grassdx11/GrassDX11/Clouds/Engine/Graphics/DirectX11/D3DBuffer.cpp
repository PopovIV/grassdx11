...
D3DBuffer::D3DBuffer(Renderer& renderer, std::string debugName)	: buffer(nullptr), renderer(renderer), debugName(debugName){}
...
void D3DBuffer::map(D3D11_MAPPED_SUBRESOURCE& inputMappedSubresource) {
	HRESULT result = this->renderer.getDeviceContext()->Map(this->buffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &inputMappedSubresource);
