...
bool ComputeShader::createFromFile(std::string path) {
	...
	if (this->device->GetFeatureLevel() < D3D_FEATURE_LEVEL_11_0) {
		Log::error(	"This GPU does not support shader model 5.0, compute shader will not be loaded.");
	...
	HRESULT result = this->device->CreateComputeShader(	csData.data(), csData.size(), nullptr, &this->computeShader);
	if (FAILED(result)) {
...
void ComputeShader::run() {
	...
	this->deviceContext->CSSetUnorderedAccessViews(	0, this->unorderedAccessViews.size(), VECTOR_ADDRESS(this->unorderedAccessViews), NULL);
	this->deviceContext->CSSetConstantBuffers(0, this->constantBuffers.size(), VECTOR_ADDRESS(this->constantBuffers));

	// Run
	this->deviceContext->Dispatch(this->threadGroupX, this->threadGroupY, this->threadGroupZ);
