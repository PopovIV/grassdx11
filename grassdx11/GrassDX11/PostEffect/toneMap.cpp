...
bool ToneMap::Initialize(ID3D11Device* device, int textureWidth, int textureHeight) {
    // Initialize the vertex and pixel shaders.
    bool result = InitializeShader(device, L"PostEffect/ToneMapVertexShader.hlsl", L"PostEffect/ToneMapPixelShader.hlsl");
    if (!result) {
        return false;
    }

    m_averageLuminance = new AverageLuminance();
    bool res = m_averageLuminance->Initialize(device, textureWidth, textureHeight);
    if (!res) {
        return false;
    }
...
bool ToneMap::InitializeShader(ID3D11Device* device, const wchar_t* vsFilename, const wchar_t* psFilename) {
