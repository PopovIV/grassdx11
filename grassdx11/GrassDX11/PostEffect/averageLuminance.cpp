...
bool AverageLuminance::Initialize(ID3D11Device* device, int textureWidth, int textureHeight) {
    ...
    result = InitializeShader(device, L"PostEffect/ToneMapVertexShader.hlsl", L"PostEffect/CopyPixelShader.hlsl", L"PostEffect/LuminancePixelShader.hlsl");
    if (!result) {
        return false;
    }
...
bool AverageLuminance::InitializeShader(ID3D11Device* device, const wchar_t* vsFilename, const wchar_t* psCopyFilename, const wchar_t* psFilename) {
