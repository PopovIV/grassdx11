...
bool TerrainShader::Initialize(ID3D11Device* device) {
    ...
    bool result = InitializeShader(device, L"Terrain/Shaders/TerrainVertexShader.hlsl", L"Terrain/Shaders/TerrainPixelShader.hlsl", L"Terrain/Shaders/TerrainHullShader.hlsl", L"Terrain/Shaders/TerrainDomainShader.hlsl", L"Terrain/Shaders/CullShader.hlsl");
    if (!result) {
        return false;
    }
...
bool TerrainShader::Render(ID3D11DeviceContext* deviceContext, XMFLOAT4* frustumPlanes, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMFLOAT3 cameraPos, XMFLOAT3 lightDir, ID3D11ShaderResourceView* textures[], bool normalPass) {
    ...
    RenderShader(deviceContext, normalPass);

    return true;
}

// Function to initialize shader
bool TerrainShader::InitializeShader(ID3D11Device* device, const wchar_t* vsFilename, const wchar_t* psFilename, const wchar_t* hsFilename, const wchar_t* dsFilename, const wchar_t* csFilename) {
    ...
    if (SUCCEEDED(result)) {
        D3D11_BUFFER_DESC desc = {};
        desc.ByteWidth = sizeof(ScaleBufferType);
        desc.Usage = D3D11_USAGE_DEFAULT;
        desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
        desc.CPUAccessFlags = 0;
        desc.MiscFlags = 0;
        ...
        result = device->CreateBuffer(&desc, &data, &m_scaleBuffer);
        assert(SUCCEEDED(result));
    }
    if (SUCCEEDED(result)) {
        D3D11_BUFFER_DESC desc = {};
        desc.ByteWidth = sizeof(XMINT4) * TERRAIN_CHUNK_COUNT_WIDTH * TERRAIN_CHUNK_COUNT_HEIGHT;
        desc.Usage = D3D11_USAGE_DEFAULT;
        desc.BindFlags = D3D11_BIND_UNORDERED_ACCESS;
        desc.CPUAccessFlags = 0;
        desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
...
// Render function
void TerrainShader::RenderShader(ID3D11DeviceContext* deviceContext, bool normalPass) {
